...
- [4. ネットワーク設計](#4-ネットワーク設計)
  ...
  - [4.2. OS内ファイアウォール](#42-os内ファイアウォール)
...
運用管理の簡素化と二重管理によるトラブル防止のため、OS レイヤーのパケットフィルタは使用しない。
ネットワークアクセス制御は OCI レイヤーの「セキュリティ・リスト」にて一元管理する。

| 対象 | 状態 | 備考 |
| ---- | ---- | ---- |
| ufw | `inactive`（無効） | パッケージは導入するがサービスは停止する |
| `iptables-persistent` / `netfilter-persistent` | 未導入（purge 済み） | OS 再起動時に保存済みルールが復元されるのを防ぐ |
| iptables の全拒否ルール | 削除 | OCI の Ubuntu イメージが既定で持つ `REJECT --reject-with icmp-host-prohibited` を INPUT / FORWARD チェインから削除する |

Kubernetes の kube-proxy およびコンテナネットワークが生成する iptables ルールは管理対象外とし、
上表の全拒否ルールのみを削除対象とする。
...
データベースのデータのみ、論理バックアップ（ダンプ）を取得する（[MySQL詳細設計書](MySQL詳細設計書.md)）。

### 8.2. リストア（環境再構築）

- **IaC アプローチ**: インスタンス障害やデータ紛失時は、Terraform によるリソース再作成および Ansible による構成管理を再実行することで環境を復元する。
- **データ永続性**:
  - データベースのデータは、日次のダンプ（[MySQL詳細設計書](MySQL詳細設計書.md)）を取り込んで復元する。最後のダンプ取得以降の更新は失われる。データベース以外のインスタンス内データは永続化管理を行わず、IaC による環境再構築のみを行う。
